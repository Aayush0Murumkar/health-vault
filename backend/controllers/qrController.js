...
const generateQR = async (req, res, next) => {
  try {
    ...
    const emergencyData = {
      patientId: user._id, name: user.name, bloodGroup: user.bloodGroup, allergies: user.allergies,
      chronicDisease: user.chronicDisease, currentMedication: user.currentMedication,
    ...
    res.json({ success: true, qrImageUrl, encryptedData });
...
const scanQR = async (req, res, next) => {
  try {
    ...
    if (!encryptedData) return res.status(400).json({ success: false, message: 'Encrypted data required' });
    const key = process.env.ENCRYPTION_KEY || 'default_encryption_key_12345678';
    let decryptedData;
    try { decryptedData = JSON.parse(decrypt(encryptedData, key)); } catch (e) { return res.status(400).json({ success: false, message: 'Invalid data' }); }
    if (decryptedData.patientId) {
      await EmergencyLog.create({ patientId: decryptedData.patientId, scannedBy: scannedBy || 'Anonymous', location: location || 'Unknown', accessedData: decryptedData });
    }
